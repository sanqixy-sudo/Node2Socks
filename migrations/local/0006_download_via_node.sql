-- Rebuild subscriptions to widen download_mode CHECK with the 'node' option
-- (download through a live proxy node via the local Core) and to add the
-- download_node_id column pointing at the chosen catalog node.
...
CREATE TABLE subscriptions_new (
    ...
    download_mode TEXT NOT NULL DEFAULT 'direct'
        CHECK (download_mode IN ('direct', 'system', 'custom_http', 'custom_socks5', 'node')),
    user_agent TEXT,
    proxy_url_cipher BLOB,
    download_node_id TEXT
);
